...
class RerankingSettings(BaseModel):
    ...
    cache_l1_ttl_seconds: int = Field(
        default=300, description="TTL for L1 (exact match) cache"
    )
    cache_l1_maxsize: int = Field(
        default=1000, description="Max entries in L1 cache per process"
    )
