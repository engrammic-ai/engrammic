...
class TestHybridRetrieval:
    ...
    async def seeded_collection(
        ...
        """Ensure collection exists (hybrid mode) and seed corpus points."""
        await qdrant_client.ensure_collection(hybrid=True)
        ...
        for i, (doc_id, content) in enumerate(_CORPUS):
            ...
            points.append(
                PointStruct(
                    ...
                    payload={"silo_id": silo_id, "content": content},
                )
            )

        await raw_client.upsert(collection_name=COLLECTION_NAME, points=points)
        yield silo_id

        # Cleanup: delete seeded points.
        from qdrant_client.models import PointIdsList

        await raw_client.delete(
            collection_name=COLLECTION_NAME,
            points_selector=PointIdsList(points=[doc_id for doc_id, _ in _CORPUS]),
        )
    ...
    async def test_sparse_search_returns_results(
        ...
        result_ids = await self._run_search(
            qdrant_client,
            silo_id,
            vector=[],  # dense vector unused in sparse-only mode
            search_mode="sparse",
            sparse_indices=sparse_indices,
            sparse_values=sparse_values,
        )
    ...
    async def test_hybrid_fallback_to_dense_when_no_sparse(
        ...
        result_ids = await self._run_search(
            qdrant_client, silo_id, query_dense, search_mode="hybrid"
        )
        assert len(result_ids) > 0
