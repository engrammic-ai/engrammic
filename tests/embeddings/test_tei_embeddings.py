...
@pytest.mark.asyncio
async def test_tei_embed_empty_list(tei_service: TEIEmbeddingService) -> None:
    """Empty input should return empty list without HTTP call."""
    with patch.object(tei_service._client, "post", new_callable=AsyncMock) as mock_post:
        result = await tei_service.embed([])
...
@pytest.mark.asyncio
async def test_tei_embed_partial_cache_hit(
    tei_service: TEIEmbeddingService,
    mock_cache: AsyncMock,
) -> None:
    """Partial cache hit should only call HTTP for uncached texts."""
    mock_cache.get.side_effect = [
        [0.1, 0.2, 0.3],  # hit for text1
        None,  # miss for text2
        [0.7, 0.8, 0.9],  # hit for text3
    ]
    ...
    with patch.object(tei_service._client, "post", new_callable=AsyncMock) as mock_post:
        mock_post.return_value = mock_response
        result = await tei_service.embed(["text1", "text2", "text3"])
        ...
        mock_post.assert_called_once()
        call_args = mock_post.call_args
        assert call_args[1]["json"] == {"inputs": ["text2"]}
