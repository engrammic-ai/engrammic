...
\hypersetup{
    ...
    pdftitle={The Case for Externalized Epistemics},
...
% TITLE
% ------------------------------------------------------------
\title{The Case for Externalized Epistemics:\\Why Agent Knowledge Must Live Outside the Model}
...
\begin{enumerate}[noitemsep]
    \item \textbf{The Externalized Epistemics Thesis}: A formal argument, grounded in information theory, optimization theory, interpretability research, distributed systems, and neuroscience, that epistemic state must be external to inference.

    \item \textbf{Epistemic Augmented Generation (EAG)}: A four-layer architecture implementing externalized epistemics with write-time coherence enforcement and AGM-compliant belief revision.
...
The remainder of the paper is organized as follows. Section~\ref{sec:thesis} presents the Externalized Epistemics Thesis with evidence from five domains. Section~\ref{sec:eag} describes the EAG architecture. Section~\ref{sec:eval} evaluates the system. Section~\ref{sec:extensions} discusses extensions to world models and multi-agent settings.
...
The thesis: any system requiring durable, auditable, multi-agent-coherent beliefs must externalize epistemic state. Evidence from five domains supports this.
...
% ------------------------------------------------------------
% 3. EPISTEMIC AUGMENTED GENERATION
% ------------------------------------------------------------
\section{Epistemic Augmented Generation}
\label{sec:eag}
...
\textbf{Epistemic Augmented Generation (EAG)} externalizes not just content but \textit{epistemic structure}: the distinctions between observations, claims, facts, and beliefs; the evidence requirements for each; the revision protocols when beliefs change.

\subsection{Premise and Axioms}

EAG rests on a foundational premise and four axioms that map epistemological concepts to computable primitives.
...
EAG operationalizes the axioms through four primitives, implemented as pure functions over structured claims of the form $(subject, predicate, object) + rationale$:

\begin{table}[H]
\centering
\caption{EAG primitives and their determinism.}
...
Provenance & Fully & Transaction invariant: DERIVED\_FROM exists at commit \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}[noitemsep]
    \item[\textbf{I1}] Every Fact has $\geq 1$ DERIVED\_FROM edge to a Memory-layer source
    \item[\textbf{I2}] Every Belief has $\geq N$ SYNTHESIZED\_FROM edges to Facts (threshold $N = 3$)
    \item[\textbf{I3}] Every Commitment has $\geq 1$ ABOUT edge and $\geq 1$ DECLARED\_BY edge
    \item[\textbf{I4}] No cycles in provenance edges (layer ordering enforces: Knowledge $\to$ Memory; Wisdom $\to$ Knowledge; never forward)
    \item[\textbf{I5}] SUPERSEDES edges require typed reason $\in \{\text{contradiction}, \text{evidence\_shift}, \text{author\_update}\}$
\end{enumerate}
...
EAG distinguishes four primary epistemic types organized into four cognitive layers:
...
EAG enforces coherence at write time, not read time. Every write passes through a validation gate:
...
When facts change, derived beliefs must be re-evaluated. EAG implements \textbf{supersession chains}:

\begin{enumerate}[noitemsep]
    \item Create new node with updated content
    \item Add SUPERSEDES edge to old node (with typed reason)
...
EAG enables multi-agent coordination by providing:
...
CITE (Context In Tiered Epistemology) implements EAG as a four-layer architecture:

\begin{figure}[H]
\centering
\begin{tikzpicture}[
    layer/.style={draw, rounded corners, minimum width=9cm, minimum height=1.0cm, align=center, font=\small\bfseries},
    arrow/.style={->, thick, >=stealth}
]
    \node[layer, fill=blue!10] (intel) at (0,3.6) {Intelligence \normalfont(session-scoped)};
    \node[layer, fill=purple!10] (wisdom) at (0,2.4) {Wisdom \normalfont(evidence-gated)};
    \node[layer, fill=green!10] (know) at (0,1.2) {Knowledge \normalfont(supersession)};
    ...
    \draw[arrow] (know.east) -- ++(1.0,0) |- node[pos=0.25, right, font=\footnotesize] {synthesis} (wisdom.east);
    \draw[arrow, dashed] (intel.west) -- ++(-1.0,0) |- node[pos=0.25, left, font=\footnotesize] {trace} (mem.west);
\end{tikzpicture}
\caption{CITE four-layer architecture.}
\label{fig:layers}
\end{figure}
...
The reference implementation uses Memgraph (graph store) and Qdrant (vector store), exposed via MCP server with operations: \texttt{remember}, \texttt{learn}, \texttt{recall}, \texttt{trace}.
...
\begin{enumerate}[noitemsep]
    ...
    \item \textbf{Supersession preserves audit}: SUPERSEDES edges function identically in latent space
\end{enumerate}
...
\textbf{EAG/CITE} & \textbf{4 + meta} & \textbf{Write-gate} & \textbf{Chains} & \textbf{Full DAG} \\
\bottomrule
\end{tabular}
\end{table}
...
EAG's contribution: four-layer epistemic ontology with write-time coherence, AGM-compliant revision, and dependency propagation.
...
We have argued that externalized epistemics is not an engineering convenience but an architectural requirement. Evidence from information theory (capacity limits), optimization theory (catastrophic forgetting), interpretability (non-auditability), distributed systems (multi-agent coherence), and neuroscience (staged consolidation) converges on the same conclusion: epistemic state must live outside the models doing inference.

Epistemic Augmented Generation implements this requirement with stratified epistemic types, write-time coherence enforcement, and dependency propagation. The framework extends to latent-space world models, opening a research direction toward epistemically-grounded JEPA architectures.
...
Chen, X., et al. (2026).
\newblock The Illusion of Interpretability in Neural Networks.
\newblock \textit{bioRxiv}. \url{https://biorxiv.org}
...
Zhang, H., et al. (2025).
\newblock Memory in LLM-based Multi-agent Systems.
\newblock \textit{TechRxiv}. \url{https://researchgate.net}
...
\textit{Inclusion}: Revision propagates only through $\text{deps}(n)$. Nodes not in the transitive closure of $\text{deps}$ are unchanged.
\end{proof}
