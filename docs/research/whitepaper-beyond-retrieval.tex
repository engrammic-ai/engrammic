...
\hypersetup{
    ...
    pdftitle={Beyond Retrieval: Epistemic Augmented Generation for Coherent Agent Memory},
...
% TITLE
% ------------------------------------------------------------
\title{Beyond Retrieval:\\Epistemic Augmented Generation for Coherent Agent Memory\\[0.4em]{\large\normalfont Treating agent memory as epistemology, not retrieval.}}
...
Long-running agents violate these assumptions. Consider an agent that records ``the API uses OAuth2'' on day one, then encounters documentation stating ``the API uses API keys'' on day two. Without epistemic structure, both statements persist with equal standing, and beliefs derived from the first (downstream tool configurations, access patterns) are never revisited. Worse, a hallucinated claim stored in one session can resurface weeks later as apparent ground truth, with no mechanism to distinguish it from verified knowledge.
...
\begin{enumerate}[noitemsep]
    \item A \textbf{formal framework for epistemic agent memory} (EAG) that extends Roynard's four-layer proposal with formal definitions of epistemic types, warrant functions, and coherence invariants, including a proof that the revision protocol satisfies AGM belief revision postulates (Theorem~\ref{thm:agm}).
    \item The \textbf{CITE architecture}, which implements EAG with write-time coherence enforcement, supersession chains, and dependency propagation.
...
The remainder of the paper is organized as follows. Section~\ref{sec:background} reviews formal epistemology and existing agent memory systems. Section~\ref{sec:eag} presents the EAG formal framework, and Section~\ref{sec:cite} describes the CITE architecture that implements it. Section~\ref{sec:eval} evaluates the system on coherence maintenance tasks, and Section~\ref{sec:discussion} discusses extensions to world models and multi-agent settings.
...
In Theorem~\ref{thm:agm} we show that EAG's supersession operation satisfies both postulates.
...
Several recent findings in LLM reliability directly motivate EAG's design choices.
...
The layers define what epistemic states exist; the \textbf{transitions} define how information moves between them. An EAG implementation that omits any transition class cannot maintain the invariants of Section~\ref{sec:cite}.

\begin{table}[H]
\centering
\caption{EAG transitions between epistemic layers.}
...
\textbf{Revision propagation.} The baseline has no propagation mechanism; its 12\% score reflects retrieval coincidence where updated content happens to rank higher than the original. CITE walks dependency graphs explicitly via Algorithm~\ref{alg:propagate}, achieving 87\%.
...
The EAG type system is representation-agnostic, and we conjecture that its principles extend naturally to latent-space representations:
\begin{enumerate}[noitemsep]
...
EAG's distinguishing contribution relative to these systems is the combination of a four-layer epistemic ontology, write-time coherence enforcement, AGM-compliant revision, and dependency propagation.
...
Engrammic implements EAG/CITE as two open-source packages under Apache 2.0.

\textbf{engrammic-primitives}\footnote{\url{https://pypi.org/project/engrammic-primitives/}} defines the schema layer: epistemic types, edge types, confidence propagation, and promotion rules. It is intended as a shared dependency for any system implementing EAG semantics.
