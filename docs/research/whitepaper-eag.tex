...
% TITLE
% ------------------------------------------------------------
\title{\textbf{Epistemic Augmented Generation}\\
    {\large A Tiered Epistemology for Agent Memory}}

\author[1]{Aliasgar Khimani}
\affil[1]{Engrammic Labs\\
    \href{mailto:aliasgar.khimani@engrammic.ai}{aliasgar.khimani@engrammic.ai}\\
    \href{https://engrammic.ai}{engrammic.ai}}
...
Roynard~\cite{roynard2026} identifies this as a \textit{category error}: RAG and naive memory systems apply one persistence model (typically decay) uniformly to information with fundamentally different persistence semantics. A decaying Slack message is correct; a decaying validated fact is a bug. This critique extends to cognitive architectures like CoALA~\cite{sumers2023coala} and JEPA~\cite{lecun2022path}, which lack an explicit Knowledge layer with proper epistemic semantics.
...
\begin{enumerate}[noitemsep]
    ...
    \item \textbf{Open-source implementation} under Apache 2.0 at \url{https://github.com/engrammic-ai/engrammic}.
\end{enumerate}
...
\textbf{mem0}~\cite{mem0} achieves 94.4\% on LongMemEval at 6.9k tokens but uses ADD-only architecture. Production failures are documented: Issue \#4573 reports 97.8\% junk rate with 808 duplicates; Issue \#4896 (closed ``not planned'') shows semantic contradictions cannot be resolved; Issue \#5330 shows no stale memory decay.

\textbf{Zep/Graphiti}~\cite{graphiti2025} implements bi-temporal validity (created\_at, valid\_at, invalid\_at) with LLM-based edge invalidation and provenance to source episodes. However, it lacks epistemic layering and evidence-gated promotion.

\textbf{MemGPT}~\cite{memgpt2023} provides tiered working/archival memory inspired by operating systems but without epistemic distinction between memory types.

\textbf{Hindsight}~\cite{hindsight} uses four memory networks (World, Experience, Opinion, Observation) with on-demand synthesis. Gap: no live truth maintenance or dependency propagation.
...
The layers define what exists; the \textbf{transitions} define what moves. An EAG implementation is the sum of its transition workers.
...
T4 & Wis $\to$ Wis (revise) & evidence shift $\geq M\%$ & signal & new \texttt{SUPERSEDES} old \\
T5 & Int $\to$ Know (consensus) & $K$ chains, $J$ agents & lazy & \texttt{PROMOTED\_FROM} \\
T6 & Int $\to$ Mem (trace) & chain completes & batched & \texttt{TRACED\_FROM} \\
T7 & Int $\to$ Wis (commit) & agent declares & eager & \texttt{DECLARED\_BY} \\
T8 & Mem $\to \bot$ (decay) & time-based & query-time & N/A \\
T9 & Any $\to$ deleted & age $>$ threshold & scheduled & N/A \\
T13 & Int $\to$ Wis (crystallize) & agent crystallizes & eager & \texttt{CRYSTALLIZED\_INTO} \\
T14 & Any $\to$ tombstone & agent forgets & eager & timestamp \\
T15 & tomb $\to$ restored & cancel in window & eager & cleared \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}[noitemsep]
    \item \textbf{Eager}: correctness-critical (T2 supersede, T7 commit, T14 forget)
    \item \textbf{Signal-driven}: optimization, heat-ranked (T1 extract, T3 synthesize)
    \item \textbf{Lazy/batched}: housekeeping (T5 consensus, T6 trace, T8/T9 decay)
\end{itemize}

The transitions are the architecture: if you know the four layers but not the transitions, you cannot build EAG.
...
INV7 & Commitments have DECLARED\_BY & T7, T13 & write \\
INV8 & Cancel window bounded & T15 & cancel \\
\bottomrule
\end{tabular}
\end{table}
...
LeCun's JEPA architecture~\cite{lecun2022path} explicitly specifies external memory. Every Vision-Language-Action paper (MemoryVLA, EchoVLA, MemER) implements external retrieval. Yet the agent memory market (\$6.27B) is 100\% text-focused---no company builds latent-space memory for world models.
...
\textbf{engrammic-primitives} (Apache 2.0): Schema library defining epistemic types, edge types, confidence propagation, and promotion rules. Available on PyPI: \texttt{pip install engrammic-primitives}.

\textbf{engrammic} (Apache 2.0): Full backend with MCP server, FastAPI admin, graph store, vector store, and SAGE pipeline (background synthesis).
...
\begin{center}
\small
Engrammic Labs · Apache 2.0 · \href{https://github.com/engrammic-ai/engrammic}{github.com/engrammic-ai/engrammic}
\end{center}
